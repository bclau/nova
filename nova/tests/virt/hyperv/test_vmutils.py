...
class VMUtilsTestCase(test.NoDBTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        super(VMUtilsTestCase, cls).setUpClass()
        cls._mock_wmi = mock.MagicMock()
        cls._wmi_patcher = mock.patch.dict(sys.modules, wmi=cls._mock_wmi)
        cls._platform_patcher = mock.patch('sys.platform', 'win32')
        cls._platform_patcher.start()
        cls._wmi_patcher.start()
        reload(vmutils)

    @classmethod
    def tearDownClass(cls):

        super(VMUtilsTestCase, cls).tearDownClass()
        cls._wmi_patcher.stop()
        cls._platform_patcher.stop()
        reload(vmutils)
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._set_vm_vcpus')
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._set_vm_memory')
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils.'
                '_get_new_resource_setting_data')
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._modify_virt_resource')
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._get_nic_data_by_name')
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._get_new_setting_data')
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._wait_for_job')
