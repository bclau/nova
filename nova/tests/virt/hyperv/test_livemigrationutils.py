...
class LiveMigrationUtilsTestCase(test.NoDBTestCase):
    ...
    def test_live_migrate_vm_disk(self):
        self._test_live_migrate_vm(self._RESOURCE_TYPE_DISK,
                                   self._RESOURCE_SUB_TYPE_DISK)

    def test_live_migrate_vm_vhd(self):
        self._test_live_migrate_vm(self._RESOURCE_TYPE_VHD,
                                   self._RESOURCE_SUB_TYPE_VHD)

    def _test_live_migrate_vm(self, resource_type, resource_sub_type):
        original_vmutils = vmutilsv2.VMUtilsV2
        original_volutils = volumeutilsv2.VolumeUtilsV2
        mock_vm_utils_remote = mock.MagicMock()
        mock_vol_utils_remote = mock.MagicMock()
        vmutilsv2.VMUtilsV2 = mock.MagicMock(return_value=mock_vm_utils_remote)
        volumeutilsv2.VolumeUtilsV2 = mock.MagicMock(
        ...
        vmutilsv2.VMUtilsV2 = original_vmutils
        volumeutilsv2.VolumeUtilsV2 = original_volutils
