...
class BaseVolumeUtilsTestCase(test.NoDBTestCase):
    ...
    def setUpClass(cls):
        ...
        cls._mock_winreg = mock.MagicMock()
        cls._wmi_patcher = mock.patch.dict(sys.modules, wmi=cls._mock_wmi)
        cls._winreg_patcher = mock.patch.dict(sys.modules,
                                              _winreg=cls._mock_winreg)
        cls._platform_patcher = mock.patch('sys.platform', 'win32')
        cls._platform_patcher.start()
        cls._wmi_patcher.start()
        cls._winreg_patcher.start()
        reload(basevolumeutils)
        super(BaseVolumeUtilsTestCase, cls).setUpClass()

    @classmethod
    def tearDownClass(cls):
        cls._winreg_patcher.stop()
        cls._wmi_patcher.stop()
        cls._platform_patcher.stop()

        # children should be reloaded as well.
        reload(basevolumeutils)
        reload(volumeutils)
        reload(volumeutilsv2)
        super(BaseVolumeUtilsTestCase, cls).tearDownClass()
    ...
    def _test_get_iscsi_initiator(self, winreg_method, expected):
        ...
        with mock.patch.object(basevolumeutils, '_winreg') as mock_winreg:
    ...
    def test_volume_in_mapping(self, mock_driver):
        mock_driver.block_device_info_get_mapping = mock.MagicMock(
