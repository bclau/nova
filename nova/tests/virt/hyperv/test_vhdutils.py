...
class VHDUtilsTestCase(test.NoDBTestCase):
    ...
    def _mocked_get_internal_vhd_size(self, root_vhd_size, vhd_type):
        mock_vhd_info = mock.MagicMock(return_value={'Type': vhd_type})
        mock_format = mock.MagicMock(return_value=constants.DISK_FORMAT_VHD)
        mock_vhd_blk_size = mock.MagicMock(
            return_value=self._FAKE_DYNAMIC_BLK_SIZE)

        with mock.patch.multiple(self._vhdutils,
                                 get_vhd_info=mock_vhd_info,
                                 get_vhd_format=mock_format,
                                 _get_vhd_dynamic_blk_size=mock_vhd_blk_size):
    ...
    def test_get_internal_vhd_size_by_file_size_unsupported(self):
        root_vhd_size = 20 * 1024 ** 3
        mock_info = mock.MagicMock(return_value={'Type': self._FAKE_BAD_TYPE})
        with mock.patch.multiple(self._vhdutils,
                                 get_vhd_info=mock_info,
                                 get_vhd_format=mock.MagicMock()):
