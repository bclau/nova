...
class HyperVAPITestCase(HyperVAPIBaseTestCase):
    ...
    def _test_live_migration(self, test_failure=False,
        ...
        self._mox.StubOutWithMock(volumeops.ISCSIVolumeDriver,
                                  'logout_storage_targets')
        ...
        if not unsupported_os:
            ...
            if with_volumes:
                fake_targets = {fake_target_iqn: [fake_target_lun]}
                m.AndReturn(fake_targets)

                volumeops.ISCSIVolumeDriver.logout_storage_targets(
                    fake_targets)
            else:
                m.AndReturn({})
