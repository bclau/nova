...
class VolumeUtilsV2TestCase(test_basevolumeutils.BaseVolumeUtilsTestCase):
    ...
    def setUp(self):
        ...
        self._volutils._conn_storage = mock.MagicMock()

    @mock.patch.object(volumeutilsv2, 'utils')
    def test_login_storage_target(self, mock_utils):
        portal = self._volutils._conn_storage.MSFT_iSCSITargetPortal
        target = self._volutils._conn_storage.MSFT_iSCSITarget

        mock_utils.parse_server_string.return_value = (
    ...
    def test_execute_log_out(self):
        ...
        mock_class = self._volutils._conn_storage.MSiSCSIInitiator_SessionClass
        mock_class.return_value = [sess]
        self._volutils._conn_storage.MSFT_iSCSITarget.return_value = [target]
        self._volutils._conn_storage.MSFT_iSCSISession.return_value = [
            iscsi_session]
