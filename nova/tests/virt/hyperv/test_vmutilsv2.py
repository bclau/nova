...
class VMUtilsV2TestCase(test.NoDBTestCase):
    ...
    @mock.patch('nova.virt.hyperv.vmutils.VMUtils._get_vm_disks')
    def test_enable_volume_metrics_collection(self, mock_get_vm_disks):
        self._lookup_vm()
        mock_svc = self._vmutils._conn.Msvm_MetricService()[0]

        mock_volume = mock.MagicMock()
        mock_volume.path_.return_value = self._FAKE_RES_PATH
        mock_volume.HostResource = [self._FAKE_MOUNTED_DISK_PATH]
        mock_get_vm_disks.return_value = ([], [mock_volume])

        self._vmutils.enable_volume_metrics_collection(
            self._FAKE_VM_NAME, self._FAKE_MOUNTED_DISK_PATH)

        mock_svc.ControlMetrics.assert_called_once_with(
            Subject=self._FAKE_RES_PATH,
            Definition=None,
            MetricCollectionEnabled=self._vmutils._METRIC_ENABLED)
