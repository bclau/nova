...
class VMUtils(object):
    ...
    def create_vm(self, vm_name, memory_mb, vcpus_num, limit_cpu_features,
                  dynamic_memory_ratio, vm_gen=constants.VM_GEN_1):
        """Creates a VM."""
        vs_man_svc = self._conn.Msvm_VirtualSystemManagementService()[0]

        LOG.debug('Creating VM %s', vm_name)
        vm = self._create_vm_obj(vs_man_svc, vm_name, vm_gen)
    ...
    def _create_vm_obj(self, vs_man_svc, vm_name, vm_gen=constants.VM_GEN_1):
    ...
    def _get_free_controller_slot(self, scsi_controller_path):
        ...
        raise HyperVException("Exceeded the maximum number of slots")
    ...
    def _get_attached_disks_query_string(self, scsi_controller_path):
        return ("SELECT * FROM Msvm_ResourceAllocationSettingData WHERE ("
                ...
                "Parent = '%(parent)s'" % {
                    ...
                    'res_sub_type_virt': self._DISK_RES_SUB_TYPE,
    ...
    def attach_ide_drive(self, vm_name, path, ctrller_addr, drive_addr,
        ...
        self.attach_drive(vm_name, path, ctrller_path, drive_addr,
                          drive_type)
