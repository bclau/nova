...
class VMUtilsV2(vmutils.VMUtils):
    ...
    def enable_vm_metrics_collection(self, vm_name):
        ...
        for volume in volumes:
            self._enable_metrics(metric_svc, volume)
    ...
    def enable_volume_metrics_collection(self, vm_name, mounted_disk_path):
        vm = self._lookup_vm_check(vm_name)
        metric_svc = self._conn.Msvm_MetricService()[0]
        volumes = self._get_vm_disks(vm)[1]
        filtered_volumes = [v for v in volumes if
                            mounted_disk_path in v.HostResource]

        if filtered_volumes:
            self._enable_metrics(metric_svc, filtered_volumes[0])
